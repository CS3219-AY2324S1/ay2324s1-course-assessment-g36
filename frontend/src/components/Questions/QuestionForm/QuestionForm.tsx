...
export default function QuestionForm({
  ...
  const handleCheckboxChange = (input: string) => {
    if (form.categories.includes(input)) {
      setFormData({
        ...form,
        categories: form.categories.filter((item) => item !== input),
      });
    } else {
    ...
    <Stack>
      ...
      <FormControl isRequired>
        ...
        <Textarea
          ...
          placeholder="Given an m x n 2D binary grid grid which represents a map of '1's (land) and '0's (water), return the number of islands."
      ...
      <FormControl isRequired>
        ...
        <CheckboxGroup colorScheme="blue">
          <Stack spacing={5} direction="row" wrap="wrap">
            {CATEGORIES_OPTIONS.slice(0, 7).map((category) => (
              <Checkbox
                key={category}
                value={category}
          ...
          <Stack spacing={5} direction="row" wrap="wrap">
            {CATEGORIES_OPTIONS.slice(7, 12).map((category) => (
              <Checkbox
                key={category}
                value={category}
                onChange={(e) => handleChange(e, "categories")}
          ...
          <Stack spacing={5} direction="row" wrap="wrap">
            {CATEGORIES_OPTIONS.slice(12).map((category) => (
              <Checkbox
                key={category}
                value={category}
                onChange={(e) => handleChange(e, "categories")}
